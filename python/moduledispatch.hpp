...
// Every ModuleDispatch<ISA::X> specialization above must be guarded by the
// matching HAVE_ISA_X macro (not e.g. a different, wider ISA's macro):
// otherwise a compiled, working pyhogpp_x object library ends up with no way
// to be reached by ModuleInitializer, and requesting HOGPP_DISPATCH=X fails
// even though X is actually available.
#if defined(HAVE_ISA_AVX)
static_assert(ModuleDispatchSupported<ISA::AVX>,
              "ModuleDispatch<ISA::AVX> must be defined whenever "
              "HAVE_ISA_AVX is defined");
#endif // defined(HAVE_ISA_AVX)

#if defined(HAVE_ISA_AVX2)
static_assert(ModuleDispatchSupported<ISA::AVX2>,
              "ModuleDispatch<ISA::AVX2> must be defined whenever "
              "HAVE_ISA_AVX2 is defined");
#endif // defined(HAVE_ISA_AVX2)

} // namespace pyhogpp

#endif // PYTHON_MODULEDISPATCH_HPP
