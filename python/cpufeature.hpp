...
struct CPUFeature<ISA::SVE128> : CPUFeature<ISA::SVE>
{
    ...
    // A fixed-width SVE build requires the hardware vector length to match
    // exactly, unlike x86 SIMD extensions where a narrower ISA is always a
    // safe subset of a wider one: hides CPUFeature<ISA::SVE>::supported().
    [[nodiscard]] static bool supported() noexcept;
};
