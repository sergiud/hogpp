...
// PYBIND11_MODULE + mod_gil_not_used() is how pybind11 opts into the
// free-threaded (no-GIL) build. nanobind exposes the same capability but
// through its own NB_MODULE API, so this whole block will need to change on
// migration.
